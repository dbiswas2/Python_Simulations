...
def gen_model():
    ...
    f0 = lambda X: math.exp(X / (1 + X))


    R_Ac_f = smodel.SReac('R_Ac_f', vsysX, lhs = [C_c, B_c], rhs = [A_c, B_c], k = lambda X:B_c)
    R_An_f = smodel.SReac('R_An_f', vsysY, lhs = [C_n, B_n], rhs=  [0.1, A_n, B_n], k = lambda X:B_n)
